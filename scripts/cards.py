#!/usr/bin/env python3
"""
cards.py - render GitHub project cards as SVGs. Stdlib only.

These are files in your own repo, so they render as long as GitHub renders.

    python scripts/cards.py --user Dhruv-Sharma29 --projects assets/projects.json \
        --out assets --repo-only

Writes one card per repo listed in assets/projects.json, as
<out>/card-<repo>-{dark,light}.svg. The optional default stats-card mode is
retained for compatibility; the profile workflow uses --repo-only.
...
def main(argv=None):
    ...
    p.add_argument("--repo-only", action="store_true",
                   help="render project cards without generating the unused stats card")
